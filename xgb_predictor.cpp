...
class xgb_predictor {
public:
    //compute score for a single tree
...
int main ()
{
    ...
    // user needs to customize the feature names below, remove it if all your data featrure names have no space between strings
    map_features = {{"ab_ab", "ab ab"}, 
                    {"cd_cd", "cd cd"}, 
                    {"ef_ef", "ef ef"}
                    };
    unordered_map <string, double> obj_feature_values;
    string in_file = "xgb_model.txt"; //model file generated by Sciki-learn library from Python ML training
    string feature_data_file = "feature_data.csv";
